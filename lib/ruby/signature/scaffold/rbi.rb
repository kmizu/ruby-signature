  ...
  module Signature
    module Scaffold
      class RBI
        ...
        def push_class(name, super_class)
        ...
        def process(node, outer: [])
          case node.type
          when :CLASS
            push_class node.children[0], node.children[1] do
              process node.children[2], outer: outer + [node]
            end
          when :MODULE
            push_module node.children[0] do
              process node.children[1], outer: outer + [node]
            end
          when :FCALL
            case node.children[0]
            when :type_member
              if outer.last.type == :CDECL
                current_module.type_params << outer.last.children[0]
              end
            when :include
          ...
          when :ALIAS
            ...
            each_child node do |child|
              process child, outer: outer + [node]
            end
          end
        end
